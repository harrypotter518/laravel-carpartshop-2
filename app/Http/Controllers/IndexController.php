...
class IndexController extends Controller
{
    public function index(Request $request){
        $search_key = str_replace("-","",$request->search_key);
        $products = Products_model::orWhere('p_code', 'like', '%'.$search_key.'%')
            ->orderBy('created_at','desc')->paginate(9);

        $search_term = $request->search_key;
        return view('frontEnd.index',compact('products','search_term'));
    }
    ...
    public function listByCat($id){
        $list_product=Products_model::where('categories_id',$id)->paginate(9);;
